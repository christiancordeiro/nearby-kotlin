...
fun findSouthwestPoint(points: List<LatLng>): LatLng {
    ...
    for (point in points) {
        if (point.latitude < southwestPoint.latitude || (point.latitude == southwestPoint.latitude && point.longitude < southwestPoint.longitude)) {
...
fun findNortheastPoint(points: List<LatLng>): LatLng {
    ...
    for (point in points) {
        if (point.latitude > northeastPoint.latitude || (point.latitude == northeastPoint.latitude && point.longitude > northeastPoint.longitude)) {
